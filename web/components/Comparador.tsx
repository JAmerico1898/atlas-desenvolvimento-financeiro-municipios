...
export default function Comparador({ municipio, nacional, municipioNome }: ComparadorProps) {
  const sources = [municipio, nacional]

  const chartData = DIMENSIONS.map(dim => {
    const norms = normalize(sources, dim.key)
    return { subject: dim.label, municipio: norms[0], nacional: norms[1] }
  })
